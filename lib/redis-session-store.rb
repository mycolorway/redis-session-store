...
class RedisSessionStore < ActionDispatch::Session::AbstractStore
  ...
  def set_session(env, sid, session_data, options = nil) # rubocop: disable MethodLength, LineLength
    expiry = (options || env.fetch(ENV_SESSION_OPTIONS_KEY))[:expire_in]
    if expiry
  ...
  class JsonSerializer
    ...
    def self.load(string)
      # object = string.start_with?(*MARSHAL_INDICATORS) ? ::Marshal.load(string) : JSON.parse(string, :symbolize_names => true)
      # _unmarshal(object)
      original = JSON.parse(string, quirks_mode: true)
    ...
    def self.dump(object)
      ...
      JSON.generate(object, quirks_mode: true)
    end
    ...
    #       object
    #     else
    #       raise "Connot correctly serialize object: #{object.inspect}"
    #     end
    #   end
    # end
    #
